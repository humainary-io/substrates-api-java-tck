...
2. **The Substrates API is Maven-resolvable.** The default dependency is:
...
This TCK defaults to Substrates API `3.0.0`. An intentional API override can be supplied with
`-Dsubstrates.api.version=<version>`; the result then applies to that API/provider combination.
