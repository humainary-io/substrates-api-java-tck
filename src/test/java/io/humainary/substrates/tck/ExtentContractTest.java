...
final class ExtentContractTest {
  ...
  /// enclosure(Consumer) rejects a null consumer even when there is no enclosure.
  @Test
  void enclosure_rootWithNullConsumer_throwsNullPointerException() {

    final var root = TestExtent.root("root");
    final var child = root.child("child");

    assertThrows(NullPointerException.class, () -> root.enclosure(null));
    assertThrows(NullPointerException.class, () -> child.enclosure(null));
  ...
  /// foldTo traverses iteratively, so depth is not bounded by the stack.
  @Test
  void foldTo_deepHierarchy_doesNotExhaustStack() {
    ...
    final int levels = current.foldTo(_ -> 1, (count, _) -> count + 1);

    assertEquals(20_000, levels);

  }

  /// foldTo seeds the accumulator exactly once, at the root.
  @Test
  void foldTo_nestedExtent_appliesInitializerOnceAtRoot() {

    final var c = TestExtent.root("a").child("b").child("c");

    final var seeded = new ArrayList< String >();

    c.foldTo(
      first -> {
        seeded.add(first.part());
        return first.part();
      },
      (acc, next) -> acc + "." + next.part()
    );

    assertEquals(List.of("a"), seeded);
  ...
  /// A null accumulator result is passed on as the next accumulator input.
  /// It is not treated as absence and does not end the traversal.
  @Test
  void foldTo_nullAccumulatorResult_isPassedToNextApplication() {
    ...
    // seeded with "a" at the root, then null for the remaining level:
    // every level was still visited
    assertEquals(Arrays.asList("a", null), inputs);
    assertNull(result);
  ...
  /// foldTo carries a null accumulator result through, exactly as fold does.
  @Test
  void foldTo_nullAccumulatorResult_returnsNull() {

    final var c = TestExtent.root("a").child("b").child("c");

    assertNull(c.fold(TestExtent::part, (_, _) -> null));
    assertNull(c.foldTo(TestExtent::part, (_, _) -> null));
  ...
  /// path applies its mapper once per extent, with nothing discarded.
  @Test
  void path_nestedExtent_appliesMapperOncePerExtent() {

    final var c = TestExtent.root("a").child("b").child("c");

    final var mapped = new ArrayList< String >();

    final var path = c.path(
      extent -> {
        mapped.add(extent.part());
        return extent.part();
      },
      '/'
    );
  ...
  /// The spliterator and stream report the characteristics their encounter order
  /// depends on, and none that Extent cannot guarantee.
  @Test
  void spliterator_andStream_reportOrderedAndNonnullOnly() {

    final var extent =
      TestExtent.root("a").child("b").child("c");

    final var spliterator =
      extent.spliterator();

    // the complete set, so a characteristic added later cannot slip in unasserted
    assertEquals(
    ...
    assertEquals(
      spliterator.characteristics(),
      extent.stream().spliterator().characteristics()
    );
  ...
  /// The stream is lazy: a short-circuiting operation does not walk the chain.
  @Test
  void stream_findFirst_doesNotTraverseWholeChain() {

    final var walks = new AtomicInteger();

    var current = new CountingExtent("level0", null, walks);

    for (var i = 1; i < 1_000; i++) {
      current = new CountingExtent("level" + i, current, walks);
    }

    walks.set(0);

    assertEquals("level999", current.stream().findFirst().orElseThrow().part());
    assertTrue(
      walks.get() < 10,
      "findFirst walked the chain " + walks.get() + " times"
    );
  ...
  /// The documented receiver-to-root order survives parallel evaluation.
  @Test
  void stream_parallel_preservesReceiverToRootOrder() {
    ...
    final var receiver = current;

    assertEquals(
      receiver.part(),
      receiver.stream().parallel().map(TestExtent::part).findFirst().orElseThrow()
    );

    assertEquals(
      receiver.stream().map(TestExtent::part).toList(),
      receiver.stream().parallel().map(TestExtent::part).toList()
    );

  }

  // ===========================
  // Projection Convenience Tests
  // ===========================
  //
  // Appendix A.2 conveniences. These carry no @SpecRef: the specification
  // classifies foldTo, stream, enclosure(Consumer) and the path overloads as
  ...
  /// An Extent that counts enclosure traversals, for laziness assertions.

  private record CountingExtent(
    String value,
    CountingExtent parent,
    AtomicInteger walks
  ) implements Extent< CountingExtent, CountingExtent > {

    @NotNull
    @Override
    public Optional< CountingExtent > enclosure() {
