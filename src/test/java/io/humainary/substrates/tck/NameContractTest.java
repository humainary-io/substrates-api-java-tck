...
final class NameContractTest
  ...
  /// Name creation supports the guaranteed minimum depth of 16 segments,
  /// whether reached in one operation or by successive extension.
  @SpecRef("4.1")
  @Test
  void name_minimumSupportedDepth_isAccepted() {
    ...
    // reached by successive extension
    var extended = cortex.name(segments.getFirst());

    for (var i = 1; i < 16; i++) {
      extended = extended.name(segments.get(i));
    }

    assertEquals(16, extended.depth());

    // reached in a single creation operation
    final var created = cortex.name(String.join(".", segments));

    assertEquals(16, created.depth());

    // both routes MUST yield the same canonical name
    assertSame(extended, created);
    assertEquals(String.join(".", segments), created.path().toString());
