...
final class FlowContractTest
  ...
  ///
  /// Both materializations are fed **the same values**, and that is the whole
  /// design of the test. Feeding one arm 1,1 and the other 10,10 cannot see the
  /// defect it exists to catch: a single `diff` slot shared by both arms passes
  /// 1 (fresh slot), drops 1 (duplicate), passes 10 (differs from 1) and drops
  /// 10, yielding exactly `[1]` and `[10]` — the correct answer, from the broken
  /// implementation. Only a value already seen by the *other* arm distinguishes
  /// them.
  ///
  /// The phases are separated by `await`, so each arm's operator runs alone and
  /// the observed contexts are an ordered trace rather than a race. `peek` sits
  /// behind `diff`, so it records exactly the emissions that survived it.
  @SpecRef({"5.1", "6.2"})
  @Test
  void flow_sameValueAcrossCircuits_isolatesStateAndUsesOwningContexts() {
    ...
    try {
        ...
        cortex.flow(Integer.class)
          .fiber(
            cortex.fiber(Integer.class)
              .diff()
              .peek(_ -> contexts.add(cortex.current()))
          );
      ...
      firstPipe.emit(7);            // fresh slot on first  — passes
      first.await();

      secondPipe.emit(7);           // same value, second's own slot — must pass
      second.await();

      firstPipe.emit(7);            // first has already seen 7 — drops
      first.await();

      secondPipe.emit(8);           // a change on second — passes
      second.await();

      secondPipe.emit(8);           // duplicate on second — drops
      second.await();

      firstPipe.emit(8);            // first still holds 7, so this is a change
      first.await();

      assertEquals(
        List.of(7, 8),
        firstValues,
        "the first materialization's diff state must be its own"
      );

      assertEquals(
        List.of(7, 8),
        secondValues,
        "a value the other materialization has already seen must still pass here"
      );
      ...
      assertEquals(
        ...
        contexts,
        "each surviving emission runs the shared operator in its owning context"
      );
  ...
  final class WindowOperator {
    ...
    /// Window use from another thread during its callback signals temporal misuse.
    ///
    ...
    /// for it, so the thread reads the window while the callback is still running,
    /// and only the context rule can reject it.
    @SpecRef({"6.4", "6.4.1"})
    @Test
    void window_fromHelperThreadDuringCallback_throwsFault() {
      ...
      try {
        ...
        assertInstanceOf(
          Fault.class,
          failure.get(),
          "a window read from another thread must fail"
        );
