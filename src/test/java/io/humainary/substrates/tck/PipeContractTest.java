...
final class PipeContractTest
  ...
  /// followed by the marker admitted to ingress behind the cycle's first
  /// emission. The value carries its iteration, so the trace names every hop
  /// exactly once and is compared whole — a trace checked only for its length
  /// and its first three entries cannot tell the required order from a provider
  /// that recurses, reorders the later laps, or drains ingress between them.
  ...
  void dispatch_boundedMultiNodeCycle_usesQueuedTransit() {
    ...
    try {
      ...
      pipes[2] = circuit.pipe(
        value -> {
          trace.add("C:" + value);
          final var lap = iterations.incrementAndGet();
          if (lap < maxIterations) {
            pipes[0].emit(String.valueOf(lap + 1));
          }
        }
      );

      // Admitted to ingress behind the cycle's first emission by per-caller
      // FIFO, so transit priority requires the whole cycle to precede it.
      final Pipe< String > marker =
        circuit.pipe(value -> trace.add("M:" + value));
      ...
      assertEquals(
        ...
        trace,
        "the cascade runs to completion, in order, before the ingress behind it"
      );
  ...
  /// A feedback loop closed through a **subscription registration** stays
  /// iterative, as §5.3 requires of any cascading chain.
  ///
  /// The two cycle tests above close their loops by calling `Pipe#emit` from
  /// inside a receptor, and an emit is an admission however a provider
  /// dispatches. They therefore say nothing about the other way a cycle can
  /// close: a registered pipe whose own receptor feeds the channel that
  /// dispatched to it, reached through the subscriber's route list rather than
  /// through the pipe's public surface.
  ///
  /// That path is where §5.3's "staged rather than recursively invoked" is at
  /// risk, because a provider is free to deliver to a registered pipe by
  /// invoking it on the dispatching stack rather than by queueing it — a
  /// legitimate optimization for a target that cannot lead back here, and a
  /// silent stack overflow for one that can. The depth is chosen so that a
  /// provider taking the recursive route exhausts the stack rather than merely
  /// running deeper, with enough margin to survive a larger stack than the one
  /// the bound was chosen against.
  ///
  /// The assertion is the delivery count, not a thrown error: the failure mode
  /// is a truncated cycle, since a provider that isolates callback failures
  /// (§15.4) will catch its own `StackOverflowError` and carry on with fewer
  /// deliveries than the fiber's limit allows.
  @SpecRef({"5.3", "7.4"})
  @Test
  void dispatch_registeredFeedbackLoop_staysIterative() {

    final var circuit = cortex.circuit();

    try {

      // Comfortably past the depth at which a recursive provider exhausts the
      // stack, so the test keeps catching one on a machine whose stack is larger
      // than the one this bound was chosen on. Iterative dispatch is unaffected
      // by the size: the loop costs one admission per hop either way.
      final int depth = 200_000;

      final var conduit =
        circuit.conduit(
          cortex.name("feedback"),
          Integer.class
        );

      final var node = conduit.get(cortex.name("node"));
      final var delivered = new AtomicInteger();

      // Feeds every emission straight back into the channel that dispatched it,
      // bounded by the fiber rather than by anything the loop itself does.
      final Pipe< Integer > loop =
        cortex.fiber(Integer.class)
          .limit(depth)
          .pipe(node);
      ...
      conduit.subscribe(
        circuit.subscriber(
          cortex.name("feedback.observer"),
          (_, registrar) -> {
            registrar.register(_ -> delivered.incrementAndGet());
            registrar.register(loop);
          }
        )
      );
      ...
      assertEquals(
        depth + 1,
        delivered.get(),
        "a registered feedback loop must run to its bound without exhausting the stack"
      );
  ...
  void emit_blockingReceptor_returnsBeforeDeliveryCompletes() throws Exception {
    ...
    final var emitter = TestTasks.fixed(1);
  ...
  /// The class-witness form rejects a null type.
  ///
  /// Projection-only: §15.2 governs specification-designated required parameters, and the
  /// class token is not one, so this test claims no normative coverage.
  @Test
  void pipe_classWitnessNullType_throwsNullPointerException() {
    ...
    try {
      ...
      assertThrows(
        NullPointerException.class,
        () -> circuit.pipe((Class< Integer >) null)
      );
  ...
  /// The class-witness form of the discarding pipe matches the witness-free form.
  ///
  ...
  @Test
  void pipe_classWitness_matchesWitnessFreeForm() {

    final var circuitName = cortex.name("witness.pipe.circuit");
    final var circuit = cortex.circuit(circuitName);

    try {

      final var sink = circuit.pipe(Integer.class);

      assertEquals(
        circuitName.toString(),
        sink.subject().name().toString()
      );
  ...
  /// The named class-witness form is equivalent to the named witness-free form.
  ///
  /// Projection-only: the class token is a Java inference affordance the specification
  /// does not define, so this test claims no normative coverage.
  @Test
  void pipe_namedClassWitness_matchesWitnessFreeForm() {

    final var circuit = cortex.circuit();

    try {

      final var name = cortex.name("discarding");

      final var sink = circuit.pipe(name, Integer.class);
  ...
  /// The name-only form binds the supplied name and discards emissions, matching a
  /// named empty target list.
  @SpecRef({"5.3", "6.1", "16.3"})
  @Test
  void pipe_namedDiscarding_bindsSubjectNameAndDiscards() {
    ...
    try {
      ...
      final Pipe< Integer > sink = circuit.pipe(name);
      ...
      assertNotSame(
        sink,
        circuit.pipe(name),
        "a named pipe must mint a new instance rather than pool by name"
      );
  ...
  /// The named forms reject a null name.
  @SpecRef({"15.2", "16.3"})
  @Test
  void pipe_namedNullName_throwsNullPointerException() {
