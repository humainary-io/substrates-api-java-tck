...
/// Conformance tests for the SPEC §9.2 single-use Closure contract and its Java exception behavior.
/// @author William David Louth
/// @since 1.0
@SpecDoc("https://github.com/humainary-io/substrates-api-spec/blob/3.0.0/SPEC.md")
final class ClosureContractTest
  extends TestSupport {

  private Cortex cortex;

  private static BiConsumer< Subject< Pipe< String > >, Registrar< String > > watcher(
    final AtomicInteger deliveries
  ) {

    return (_, registrar) ->
      registrar.register(
        _ -> deliveries.incrementAndGet()
      );
  ...
  /// - `scope.register()` after close → raises Fault (synchronous @New on closed receiver)
  /// - `scope.scope()` after close → raises Fault (invalid hierarchy)
  ...
  /// - Exception handling (scope closed in finally, cleanup attempts to consume)
  ...
  /// Closure does not invoke after its Scope closes.
  @SpecRef("9.2")
  @Test
  void consume_afterScopeClose_doesNotInvokeConsumer() {
    ...
    final Pipe< String > pipe = conduit.get(channelName);
  ...
  /// Closure closes its resource after successful consumption.
  @SpecRef("9.2")
  @Test
  void consume_consumerReturns_closesResource() {
  ...
  /// Closure closes the resource and rethrows failure.
  @SpecRef("9.2")
  @Test
  void consume_consumerThrows_closesResourceAndRethrows() {
