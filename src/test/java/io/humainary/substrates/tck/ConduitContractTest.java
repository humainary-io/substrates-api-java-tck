...
final class ConduitContractTest
  ...
  /// The class-witness forms delegate to their witness-free equivalents.
  ///
  /// Projection-only: the delegation is a Java overload arrangement the specification does
  /// not define, so this test claims no normative coverage.
  @Test
  void conduit_witnessForms_delegateToWitnessFree() {

    final var circuit = cortex().circuit();

    try {

      final var name = cortex().name("delegating");

      assertEquals(
        name,
        circuit.conduit(name, Integer.class).subject().name()
      );

      assertEquals(
        name,
        circuit.conduit(name, Integer.class, Routing.STEM).subject().name()
      );

      assertEquals(
        circuit.subject().name(),
        circuit.conduit(Integer.class, Routing.STEM).subject().name()
      );
  ...
  /// The witness-free named routing form binds the supplied name.
  @SpecRef("10.3")
  @Test
  void conduit_witnessFreeNamedRouting_bindsSubjectName() {

    final var circuit = cortex().circuit();

    try {

      final var name = cortex().name("routed");

      final Conduit< Integer > conduit = circuit.conduit(name, Routing.STEM);

      assertEquals(
        name,
        conduit.subject().name()
      );
  ...
  /// The witness-free named form binds the supplied name and carries the call-site type.
  @SpecRef("10.3")
  @Test
  void conduit_witnessFreeNamed_bindsSubjectName() {
    ...
    try {
      ...
      final Conduit< Integer > conduit = circuit.conduit(name);
  ...
  /// Witness-free named forms reject a null name.
  @SpecRef({"10.3", "15.2"})
  @Test
  void conduit_witnessFreeNullName_throwsNullPointerException() {
    ...
    try {
      ...
      assertThrows(
        NullPointerException.class,
        () -> circuit.conduit((Name) null, Routing.PIPE)
      );
  ...
  /// The witness-free routing form uses the owning circuit's name.
  @SpecRef("10.3")
  @Test
  void conduit_witnessFreeRouting_usesCircuitName() {

    final var circuit = cortex().circuit();

    try {

      final Conduit< Integer > conduit = circuit.conduit(Routing.STEM);
  ...
  final class StemRouting {
    ...
    /// Leaf-first ordering holds when the levels register different kinds of pipe.
    ///
    /// The companion test above registers a receptor at every level, which a
    /// provider representing every registration the same way passes however it
    /// represents them. §10.3 propagates leaf-first through the whole chain, so a
    /// provider that decides per level how a level's registrations are delivered —
    /// during the ancestor walk, or as work queued behind it — can invert the two
    /// while passing that test.
    ///
    /// A Fiber-composed pipe and a plain circuit pipe are the two kinds, and both
    /// arrangements are asserted, since an order that holds only one way round is
    /// not an order.
    @SpecRef({"5.3", "10.3"})
    @Test
    void dispatch_leafEmissionWithMixedRegistrationKinds_deliversLeafBeforeAncestors() {
      ...
      try {
        ...
        final Pipe< Integer > composed =
          cortex.fiber(Integer.class)
            .pipe(circuit.pipe(_ -> order.add("composed")));

        final Pipe< Integer > plain =
          circuit.pipe(_ -> order.add("plain"));

        final var composedLeaf =
          circuit.conduit(
            cortex.name("composed-leaf"),
            Integer.class,
            STEM
          );

        composedLeaf.subscribe(
          circuit.subscriber(
            ...
            (subject, registrar) -> {
              ...
              if (name.equals("app.leaf")) {
                registrar.register(composed);
              } else if (name.equals("app")) {
                registrar.register(plain);
              }
        ...
        composedLeaf.get(cortex.name("app.leaf")).emit(1);
        circuit.await();

        assertEquals(
          List.of("composed", "plain"),
          order,
          "the leaf must be observed before its ancestor"
        );
        ...
        plainLeaf.get(cortex.name("app.leaf")).emit(2);
        circuit.await();

        assertEquals(
          List.of("plain", "composed"),
          order,
          "leaf-first must not depend on which level composes its pipe"
        );
    ...
    /// Leaf-first orders the dispatch, and each registration is delivered by its kind.
    ///
    /// §10.3 visits the leaf's registrations before its ancestor's, and §6.3 then
    /// invokes a receptor registration during the dispatch and queues a pipe
    /// registration's delivery. A receptor on the leaf and a pipe on the ancestor
    /// therefore deliver the leaf first. A pipe on the leaf and a receptor on the
    /// ancestor deliver the ancestor first: the leaf's pipe was handed the emission
    /// first, and its delivery follows its queue.
    @SpecRef({"6.3", "10.3"})
    @Test
    void dispatch_leafEmissionWithReceptorAndPipeAcrossLevels_deliversEachByItsKind() {
      ...
      try {
        ...
        final Pipe< Integer > leafPipe =
          circuit.pipe(_ -> order.add("leaf"));

        final Pipe< Integer > ancestorPipe =
          circuit.pipe(_ -> order.add("ancestor"));

        final var receptorLeaf =
          circuit.conduit(
            cortex.name("receptor-leaf"),
            Integer.class,
            STEM
          );

        receptorLeaf.subscribe(
          circuit.subscriber(
            cortex.name("observer"),
            (subject, registrar) -> {
              ...
              if (name.equals("app.leaf")) {
                ...
                registrar.register(ancestorPipe);
              }
        ...
        receptorLeaf.get(cortex.name("app.leaf")).emit(1);
        circuit.await();

        assertEquals(
          List.of("leaf", "ancestor"),
          order,
          "a receptor on the leaf runs during the dispatch, ahead of the ancestor pipe's delivery"
        );
        ...
        pipeLeaf.subscribe(
          circuit.subscriber(
            ...
            (subject, registrar) -> {
              ...
              if (name.equals("app.leaf")) {
                registrar.register(leafPipe);
              } else if (name.equals("app")) {
                registrar.register(_ -> order.add("ancestor"));
              }
        ...
        pipeLeaf.get(cortex.name("app.leaf")).emit(2);
        circuit.await();

        assertEquals(
          List.of("ancestor", "leaf"),
          order,
          "a receptor on the ancestor runs during the dispatch, ahead of the leaf pipe's delivery"
        );
    ...
    /// Threads race to look up one three-deep name and emit through whatever they receive, so an
    /// emission can leave before another thread's lookup has finished. Each thread emits its own
    /// index, which tags every delivery, so a chain observed for one emission cannot be completed by
    /// another thread's work: a lookup that published a partial ancestor chain shows up as a short
    /// group, and a lookup that published a second pipe for the name shows up as a lost identity.
    ///
    /// Concurrent STEM lookup yields one canonical pipe delivering a complete leaf-first chain.
    @SuppressWarnings("TryFinallyCanBeTryWithResources")
    @SpecRef({"10.1", "10.3"})
    @Test
    void get_sameStemNameConcurrently_deliversCompleteLeafFirstChain()
      throws Exception {
      ...
      try {
        ...
        final var leaf = cortex.name("app.tier.leaf");
        final var tier = cortex.name("app.tier");
        final var root = cortex.name("app");
        ...
        conduit.subscribe(
          circuit.subscriber(
            ...
            (subject, registrar) -> {
              ...
              registrar.register(
                emission -> deliveries.add(emission + ":" + path)
              );
        ...
        final var pipes = new ConcurrentHashMap< Integer, Pipe< Integer > >();
        ...
        try {
          ...
          for (int i = 0; i < threads; i++) {
            ...
            futures.add(
              executor.submit(() -> {
                ...
                await(start, "the concurrent STEM lookup start gate");

                final var pipe = conduit.get(leaf);

                pipes.put(value, pipe);
                pipe.emit(value);
          ...
          for (final var future : futures) {
            get(future, "a concurrent STEM lookup");
          }
        ...
        final var canonical = pipes.get(0);

        pipes.values().forEach(
          pipe -> assertSame(canonical, pipe, "concurrent STEM lookup returned a second pipe")
        );

        assertEquals(
          threads * 3,
          deliveries.size(),
          "an emission did not reach every ancestor"
        );

        for (int value = 0; value < threads; value++) {

          final var tag = value + ":";

          assertEquals(
            List.of(tag + leaf, tag + tier, tag + root),
            deliveries.stream().filter(delivery -> delivery.startsWith(tag)).toList(),
            "emission " + value + " did not reach its ancestors leaf-first"
          );
