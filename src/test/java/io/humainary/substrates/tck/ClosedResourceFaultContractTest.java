...
final class ClosedResourceFaultContractTest
  ...
  /// A Bank whose owning Circuit has closed is not itself closed, so its own open-guard passes.
  /// The lookup must still materialize a Conduit, and creating one is open-guarded on the Circuit.
  /// That guard is the one that rejects, so the fault identifies the Circuit rather than the Bank
  /// (§9.1). A lookup that the Bank could satisfy from an already-materialized name reaches no
  /// closed receiver and is left unconstrained by the specification.
  @SpecRef({"9.1", "10.4", "15.1"})
  @Test
  void get_onBankAfterCircuitClose_throwsFaultNamingCircuit() {
    ...
    // An unmaterialized name, so the lookup has to reach the Circuit's factory.
    final var fault =
      assertThrows(
        Fault.class,
        () -> bank.get(
