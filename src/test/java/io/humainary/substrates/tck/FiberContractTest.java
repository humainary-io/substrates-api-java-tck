...
final class FiberContractTest
  ...
  final class NaturalOrdering {
    ...
    // ----- Fail fast: chain-build CCE when a bound is not Comparable -----
    //
    // The probe validates the supplied bounds, not the declared E, and it
    // evaluates one direction only. A bound of type Object is not Comparable,
    // so the CCE must surface at the operator call site (no circuit, no emit)
    // — proving the eager probe inside naturalOrder(probe) fired before the
    // chain even materialized. A non-Comparable E with a Comparable bound is
    // deliberately not rejected here; see Fiber#naturalOrder.
  ...
  final class Validation {
    ...
    void streak_nonPositiveThreshold_throwsIllegalArgumentException() {
      ...
      assertThrows(
        IllegalArgumentException.class,
        () -> cortex.fiber(Integer.class).streak(0, _ -> true)
      );
