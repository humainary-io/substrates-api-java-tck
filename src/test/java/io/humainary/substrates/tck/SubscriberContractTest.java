...
final class SubscriberContractTest
  ...
  /// Registration order holds across pipe registrations of different kinds.
  ///
  /// §7.4 makes registration order normative between **pipe** registrations and
  /// leaves it unspecified only between a pipe and a receptor. The companion
  /// test above uses circuit pipes for both, which a provider representing every
  /// pipe registration the same way passes regardless. Registering a conduit
  /// channel beside a circuit pipe is what separates the two representations: a
  /// channel dispatches onward to its own subscribers, a circuit pipe runs a
  /// terminal receptor, and a provider delivering one during the dispatch walk
  /// and the other after it reorders them.
  ///
  /// Both arrangements are asserted, since an order that only holds one way
  /// round is not an order.
  @SpecRef({"6.3", "7.4"})
  @Test
  void dispatch_mixedPipeKinds_preservesRegistrationOrder() {

    final var trace = new ArrayList< String >();

    final var downstream =
      circuit.conduit(
        cortex.name("downstream"),
        Integer.class
      );

    downstream.subscribe(
      circuit.subscriber(
        cortex.name("downstream.observer"),
        (_, registrar) ->
          registrar.register(
            _ -> trace.add("channel")
          )
    ...
    final Pipe< Integer > channel = downstream.get(cortex.name("target"));
    final Pipe< Integer > direct = circuit.pipe(_ -> trace.add("direct"));

    final var channelFirst = circuit.conduit(cortex.name("a"), Integer.class);

    channelFirst.subscribe(
      circuit.subscriber(
        cortex.name("a.observer"),
        (_, registrar) -> {
          registrar.register(channel);
          registrar.register(direct);
        }
      )
    );

    channelFirst.get(cortex.name("c")).emit(1);
    circuit.await();

    assertEquals(
      List.of("channel", "direct"),
      trace,
      "channel registered first must be delivered first"
    );

    trace.clear();

    final var directFirst = circuit.conduit(cortex.name("b"), Integer.class);

    directFirst.subscribe(
      circuit.subscriber(
        cortex.name("b.observer"),
        (_, registrar) -> {
          registrar.register(direct);
          registrar.register(channel);
        }
    ...
    directFirst.get(cortex.name("c")).emit(2);
    circuit.await();

    assertEquals(
      List.of("direct", "channel"),
      trace,
      "circuit pipe registered first must be delivered first"
    );

  }

  /// Mixed registrations are dispatched in registration order, each by its kind.
  ///
  /// §6.3 invokes a receptor registration directly at its place in the dispatch,
  /// and emits to a pipe registration, whose delivery is queued. With the kinds
  /// interleaved, and each registered twice as §7.4 allows, both receptor
  /// deliveries land during the dispatch, ahead of every pipe delivery, and the
  /// pipe deliveries then follow in registration order.
  @SpecRef({"6.3", "7.4"})
  @Test
  void dispatch_mixedRegistrations_preservesPipeOrderAndDuplicateDelivery() {
    ...
    final Receptor< Integer > second = _ -> trace.add("receptor:second");
    ...
    assertEquals(
      ...
      trace,
      "receptors must run during the dispatch, and pipe deliveries follow in registration order"
    );

  }

  /// A registered pipe is delivered behind transit work accepted before the dispatch.
  ///
  /// §5.3 sequences what a circuit-context callback raises behind transit work
  /// already accepted for the current cascading chain. A receptor that emits to a
  /// subscribed channel and then to a circuit pipe has therefore accepted the
  /// second before the channel's registrations are delivered, and no
  /// representation a provider chooses for a registration changes that.
  ///
  /// The registration is varied across the two arrangements for the same reason
  /// the companion order test varies its kinds: a provider that delivers one kind
  /// during the dispatch walk and another after it would otherwise pass here by
  /// accident, and the sequencing must hold for a registration of either kind.
  @SpecRef({"5.3", "6.3", "7.4"})
  @Test
  void dispatch_registeredPipe_sequencesBehindAcceptedTransitWork() {

    final var trace = new ArrayList< String >();

    final Pipe< Integer > child = circuit.pipe(value -> trace.add("child:" + value));
    final Pipe< Integer > composed =
      cortex.fiber(Integer.class)
        .pipe(circuit.pipe(value -> trace.add("composed:" + value)));
    final Pipe< Integer > pending = circuit.pipe(value -> trace.add("pending:" + value));

    final var plain = circuit.conduit(cortex.name("plain"), Integer.class);

    plain.subscribe(
      circuit.subscriber(
        cortex.name("plain.observer"),
        (_, registrar) -> registrar.register(child)
      )
    );

    final var plainChannel = plain.get(cortex.name("target"));

    final Pipe< Integer > intoPlain =
      circuit.pipe(
        value -> {
          plainChannel.emit(value);
          pending.emit(value);
        }
      );
    ...
    assertEquals(
      List.of("pending:1", "child:1"),
      trace,
      "a registered pipe must be delivered behind work accepted before its dispatch"
    );

    trace.clear();

    final var wrapped = circuit.conduit(cortex.name("wrapped"), Integer.class);

    wrapped.subscribe(
      circuit.subscriber(
        cortex.name("wrapped.observer"),
        (_, registrar) -> registrar.register(composed)
      )
    );

    final var wrappedChannel = wrapped.get(cortex.name("target"));

    final Pipe< Integer > intoWrapped =
      circuit.pipe(
        value -> {
          wrappedChannel.emit(value);
          pending.emit(value);
        }
      );
    ...
    assertEquals(
      List.of("pending:2", "composed:2"),
      trace,
      "the sequencing must not depend on how the registered pipe was composed"
    );

  }


  /// A registered receptor runs ahead of transit work accepted before the dispatch.
  ///
  /// The companion of the test above. §6.3 invokes a receptor registration during
  /// the dispatch, so it observes the emission ahead of work the circuit accepted
  /// before the channel dispatched, while a pipe registered on the same channel is
  /// delivered behind that work. The receptor is registered after the pipe, so
  /// registration order alone cannot put it first.
  @SpecRef({"5.3", "6.3", "7.4"})
  @Test
  void dispatch_registeredReceptor_runsAheadOfAcceptedTransitWork() {

    final var trace = new ArrayList< String >();

    final Pipe< Integer > child = circuit.pipe(value -> trace.add("pipe:" + value));
    final Pipe< Integer > pending = circuit.pipe(value -> trace.add("pending:" + value));

    final var conduit = circuit.conduit(cortex.name("mixed"), Integer.class);

    conduit.subscribe(
      circuit.subscriber(
        cortex.name("mixed.observer"),
        (_, registrar) -> {
          registrar.register(child);
          registrar.register(value -> trace.add("receptor:" + value));
        }
      )
    );

    final var channel = conduit.get(cortex.name("target"));

    final Pipe< Integer > into =
      circuit.pipe(
        value -> {
          channel.emit(value);
          pending.emit(value);
    ...
    assertEquals(
      List.of("receptor:1", "pending:1", "pipe:1"),
      trace,
      "a registered receptor runs during the dispatch, and a registered pipe behind accepted work"
    );

  }


  /// Work a registered pipe raises is delivered behind the remaining registrations.
  ///
  /// §5.3 sequences what a callback raises behind transit work already
  /// accepted, and every pipe registered on the channel has its delivery
  /// accepted during the dispatch, before any of them runs. So when the first
  /// registered pipe emits, that emission joins the back of transit, behind the
  /// second pipe's delivery, however a provider groups the deliveries.
  @SpecRef({"5.3", "6.3", "7.4"})
  @Test
  void dispatch_registeredPipeEmission_followsRemainingRegistrations() {
    ...
    final var conduit = circuit.conduit(cortex.name("raising"), Integer.class);

    conduit.subscribe(
      circuit.subscriber(
        cortex.name("raising.observer"),
        (_, registrar) -> {
          registrar.register(first);
          registrar.register(second);
        }
    ...
    conduit.get(cortex.name("target")).emit(1);
    circuit.await();

    assertEquals(
      List.of("first:1", "second:1", "raised:1"),
      trace,
      "what a registered pipe raises must follow the registrations delivered with it"
    );

  }

  /// A registered pipe that throws does not prevent delivery to its siblings.
  ///
  /// §15.4 isolates each downstream pipe on a channel, and that must hold
  /// however a provider groups the pipes' deliveries.
  @SpecRef({"6.3", "15.4"})
  @Test
  void dispatch_registeredPipeThrows_preservesSiblingDelivery() {
    ...
    final var conduit = circuit.conduit(cortex.name("throwing"), Integer.class);

    conduit.subscribe(
      circuit.subscriber(
        cortex.name("throwing.observer"),
        (_, registrar) -> {
          registrar.register(good1);
          registrar.register(bad);
          registrar.register(good2);
        }
      )
    );
    ...
    assertEquals(
      List.of(10, 100),
      received,
      "a throwing registered pipe must not stop its siblings"
    );

  }


  /// Registered state-holder pipes apply behind updates accepted before the dispatch.
  ///
  /// The same §5.3 sequencing as the trace above, asserted where it decides a
  /// result rather than an order: a callback emits `1` to a channel whose
  /// registrations are a cell's and a basin's update pipes, then emits `2` to
  /// those pipes directly. The direct updates were accepted first, so `1` is
  /// applied last — the cell publishes `1`, and the basin drains `2` before `1`.
  ///
  /// This is what makes the ordering observable to a caller that never sees a
  /// dispatch: delivering a registration during the walk instead leaves the cell
  /// holding `2`.
  @SpecRef({"5.3", "6.3", "11.2"})
  @Test
  void dispatch_registeredStateHolderPipes_applyBehindAcceptedUpdates() {

    final Cell< Integer > cell = circuit.cell(0);
    final Basin< Integer > basin = circuit.basin(8);

    final var conduit = circuit.conduit(cortex.name("holders"), Integer.class);

    conduit.subscribe(
      circuit.subscriber(
        cortex.name("holders.observer"),
        (_, registrar) -> {
          registrar.register(cell.pipe());
          registrar.register(basin.pipe());
        }
      )
    );
    ...
    assertEquals(
      1,
      cell.get(),
      "the registered update is applied after the update accepted before the dispatch"
    );

    assertEquals(
      List.of(2, 1),
      retained,
      "the basin retains the accepted update ahead of the registered one"
    );
  ...
  /// A registrar used from another thread during its callback signals temporal misuse.
  ///
  /// §6.4 binds a callback-scoped handle to the circuit context that issued it as
  /// well as to the callback's duration. The callback starts a thread and waits
  /// for it, so the thread calls the registrar while the callback is still
  /// running, and only the context rule can reject it. Both overloads must throw,
  /// and neither registration may take effect.
  @SpecRef({"6.4", "6.4.1", "7.4"})
  @Test
  void register_fromHelperThreadDuringCallback_throwsIllegalState() {
    ...
    final Pipe< Long > target =
      circuit.pipe(_ -> deliveries.incrementAndGet());
    ...
    conduit.get(cortex.name("test")).emit(1L);
    circuit.await();

    assertEquals(2, failures.size(), "both overloads must reject the helper thread");

    for (final var failure : failures) {
      assertInstanceOf(IllegalStateException.class, failure);
    }

    assertEquals(0, deliveries.get(), "a rejected registration must not take effect");
