...
final class ResourceContractTest
  ...
  /// Every caller races the same resource, so at most one of them can win the transition that
  /// schedules cleanup. The losers still carry the same postcondition: each reads the cleanup
  /// counter immediately after its own closeAwait returns, so a caller that returned while cleanup
  /// was merely accepted — or not yet submitted — reads zero and fails. The final count proves the
  /// winner ran cleanup once rather than once per caller.
  ///
  /// Concurrent closeAwait callers each return after a single completed cleanup.
  @SuppressWarnings("TryFinallyCanBeTryWithResources")
  @SpecRef("9.1")
  @Test
  void closeAwait_concurrentCallers_eachReturnAfterOneCompletedCleanup()
    throws Exception {

    final var conduit = circuit.conduit(Integer.class);
    final var cleanups = new AtomicInteger();
    final var subscriber =
      circuit.< Integer > subscriber(
        cortex.name("resource.concurrent"),
        (_, _) -> {
        }
      );
    final var subscription =
      conduit.subscribe(subscriber, _ -> cleanups.incrementAndGet());

    final int callers = 8;
    final var start = new CountDownLatch(1);
    final var observed = new int[callers];
    ...
    try {
      ...
      for (int i = 0; i < callers; i++) {
        ...
        futures.add(
          executor.submit(() -> {
            await(start, "the concurrent closeAwait start gate");
            subscription.closeAwait();
            observed[index] = cleanups.get();
            return null;
          })
        );
      ...
      for (final var future : futures) {
        get(future, "a concurrent closeAwait");
      }
    ...
    assertEquals(1, cleanups.get(), "cleanup ran more than once");
    ...
    for (int index = 0; index < callers; index++) {
      ...
      assertEquals(
        1,
        observed[index],
        "closeAwait caller " + index + " returned before cleanup completed"
      );
