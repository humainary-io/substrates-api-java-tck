...
@SuppressWarnings({"TryFinallyCanBeTryWithResources", "DataFlowIssue"})
...
final class CircuitContractTest
  ...
  void await_afterTerminalClose_returnsImmediately() {
    ...
    final var waiter = TestTasks.fixed(1);
    try {
  ...
  void await_concurrentCallers_completeIndependently() throws Exception {
    ...
    final var executor = TestTasks.fixed(2);
  ...
  void await_fromExternalContext_observesPriorWork() throws Exception {
    ...
    try {
      ...
      final var executor = TestTasks.fixed(1);
      try {
  ...
  void close_pendingEmissions_returnsWithoutBlocking() {
    ...
    final var closer = TestTasks.fixed(1);
  ...
  /// Admission order survives a callback that empties the queue and refills it
  /// past any single storage tier.
  ///
  /// §5.3 sequences what a circuit-context callback raises behind transit work
  /// already accepted, and §5.4.3 requires the observed sequence to be the
  /// admission sequence. A provider that stages transit work in tiers has to keep
  /// that true across the transition where a callback consumes the last pending
  /// item and then admits a burst large enough to fill every tier again: the
  /// storage the burst ends up in is newer than the storage it started in, and a
  /// drain that keeps walking the tier it was in delivers the newest work first.
  ///
  /// The burst is deliberately large and the assertion is a total order rather
  /// than a boundary, so the test states the contract without naming any tier or
  /// capacity a provider might choose.
  @SpecRef({"5.3", "5.4.3"})
  @Test
  void dispatch_burstAfterQueueDrains_preservesAdmissionOrder() {

    final var circuit = cortex.circuit();

    try {

      final var lead = 8_192;
      final var burst = 65_536;

      final var delivered = new ArrayList< Integer >(burst);

      final Pipe< Integer > recorder = circuit.pipe(delivered::add);

      final Pipe< Integer > cascade =
        circuit.pipe(
          value -> {
      ...
      final Pipe< Integer > start =
        circuit.pipe(
          _ -> {
            ...
            for (var index = 1; index <= lead; index++) {
              cascade.emit(index);
            }
      ...
      assertEquals(
        burst,
        delivered.size(),
        "every emission of the burst must be delivered"
      );
      ...
      for (var index = 0; index < burst; index++) {
        ...
        assertEquals(
          index + 1,
          delivered.get(index),
          "delivery " + index + " is out of admission order"
        );
  ...
  void dispatch_concurrentIngress_executesCallbacksExclusively()
    ...
    try {
      ...
      final var executor = TestTasks.fixed(4);
  ...
  void dispatch_concurrentProducers_preservesValueIntegrity()
    ...
    try {
      ...
      final var executor = TestTasks.fixed(threadCount);
  ...
  /// Verifies emission integrity when subscribers dynamically subscribe and
  /// unsubscribe while emissions are ongoing.
  ///
  /// The race itself admits many legal outcomes, so it carries no oracle of its
  /// own beyond the always-subscribed observer: how many of 10,000 emissions
  /// fall inside any of 100 unsynchronized windows is not a fact the contract
  /// fixes, and asserting a number here would assert a schedule.
  ///
  /// What the contract does fix is the state the churn leaves behind. The race
  /// is therefore followed by a quiesced phase with a known admission order —
  /// `subscribe → emit(visible) → close → emit(hidden)` from one caller, whose
  /// positions §7.6.1 resolves — and the secondary's **delta** across that phase
  /// is asserted exactly. That delta discriminates the three ways churn can
  /// corrupt the subscription machinery: a route left behind by a closed
  /// subscription makes it too large, a subscriber the churn rendered
  /// unsubscribable makes it zero, and a close that no longer takes effect
  /// admits the hidden batch.
  @SpecRef({"7.6", "7.6.1"})
  @Test
  void dispatch_duringSubscriptionChanges_preservesVisibleEmissions()
    ...
    try {
      ...
      try {
        ...
        // The churn is over and drained; whatever it delivered is now fixed.
        final var churned = secondaryReceived.get();

        // A quiesced phase whose admission order per-caller FIFO decides:
        // subscribe, then the visible batch, then close, then the hidden batch.
        // No await separates close from the batch behind it, so the close takes
        // effect by its ingress position alone.
        final int visible = 25;
        final int hidden  = 25;
        ...
        assertEquals(
          churned + visible,
          secondaryReceived.get(),
          "after the churn, a fresh subscription must deliver its window exactly: "
          + "no route survives its closed subscription, and none is lost"
        );

        assertEquals(
          emissionCount + visible + hidden,
          primaryReceived.get(),
          "the always-subscribed observer receives both phases in full"
        );
  ...
  ///
  /// Strengthens [#dispatch_transitAndIngress_prioritizesTransit()] by guaranteeing that
  ...
  void dispatch_transitWithConcurrentIngress_preservesTransitPriority() throws Exception {
    ...
    try {
      ...
      final var producer = TestTasks.fixed(1);
      try {
  ...
  void emit_blockedCircuitBacklog_returnsWithoutLoss() throws Exception {
    ...
    try {
      ...
      final var submitter = TestTasks.fixed(1);
  ...
  void get_differentNamesUnderContention_returnsIndependentPipes()
    ...
    try {
      ...
      final var executor = TestTasks.fixed(10);
  ...
  void get_sameBankNameUnderContention_returnsOneCanonicalConduit()
    ...
    try {
      ...
      final var executor = TestTasks.fixed(20);
  ...
  void ingress_multipleProducers_preservesPerCallerFifo() throws Exception {
    ...
    try {
      ...
      final var producers = TestTasks.fixed(producerCount);
  ...
  /// Sibling cascades are sequenced behind transit work already accepted,
  /// so the traversal is breadth-first and not depth-first.
  ///
  /// A single-successor chain cannot establish this: 1→2→3 reads the same under
  /// either traversal. Here the first emission produces two successors and the
  /// first of those produces one of its own, which is the smallest shape the two
  /// orders disagree on.
  ///
  /// Emitting A yields A1 and A2; processing A1 yields A1a.
  ///
  /// Expected: [A, A1, A2, A1a] — A1a is sequenced behind A2, which was already
  /// accepted for this cascade when A1a was emitted.
  ///
  /// NOT: [A, A1, A1a, A2] — which would mean a successor is followed to its own
  /// depth before its sibling, the recursive model SPEC.md §5.3 excludes.
  @SpecRef("5.3")
  @Test
  void transit_siblingEmissions_sequenceBehindAcceptedWork() {
    ...
    try {
      ...
      final var pipe =
        conduit.get(cortex.name("sibling.cascade.channel"));

      conduit.subscribe(
        circuit.subscriber(
          cortex.name("sibling.cascade.subscriber"),
          (_, registrar) ->
            registrar.register(value -> {
      ...
      assertEquals(
        List.of("A", "A1", "A2", "A1a"),
        results,
        "A cascade must be traversed breadth-first, not depth-first"
      );
